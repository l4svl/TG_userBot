...
def Thanos():
    ...
    app.send_video(priton,r"C:\Users\User\PycharmProjects\TG_userBot\resource\thanos-finger-snap.mp4",caption='50% из вас будут уничтожены!')
    for i in range(int(app.get_chat_members_count(priton)//2)):
        try:
            name=random.choice(keys)
            keys.remove(name)
            ud=result.get(name)
            app.set_administrator_title(priton,ud,'Помер как лох!')
            app.restrict_chat_member(priton,ud,pyrogram.types.ChatPermissions(),datetime.now()+timedelta(minutes=15))
            app.send_message(priton,f'Пользователь {name} был успешно уничтожен, для баланса во вселенной!')
        except Exception:
            pass
    app.send_message(priton,'Рома лох попуск ебаный (имхо) азазза')
app.start()
users_id()
Thanos()
